...
struct ContentView: View {
    ...
    @State private var chart: Chart = {
        ...
        c.addIndicator(.sma, period: 20, color: ChartColor(r: 1.0, g: 0.85, b: 0.20))
    ...
    private var controls: some View {
        VStack(spacing: 8) {
            ...
            HStack(spacing: 12) {
                Text("\(chart.candleCount) candles")
                    .font(.caption.monospaced())
                Text("visible: \(chart.visibleCount)")
                    .font(.caption.monospaced())
                Spacer()
                Text("핀치/드래그/길게누름")
                    .font(.caption2)
